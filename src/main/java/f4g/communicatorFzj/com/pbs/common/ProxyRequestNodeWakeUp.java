...
/**
 * Executes a shell command to wake up a node
 * 
 *
 * @author Andre Giesler
 */
public class ProxyRequestNodeWakeUp extends ProxyRequest {
	
	/**
	 * 
	 */
	
	static Logger log = Logger.getLogger(ProxyRequestNodeWakeUp.class.getName());
	...
	public ProxyRequestNodeWakeUp(String node, String execCmd, String execDir){
		setExecCmd(execCmd);
		setExecDir(execDir);
		setNode(node);
	}
	...
	public ProxyResponse execute() throws IOException {
		...
		ProxyResponseNodeWakeUp proxyResponsePowerOn = new ProxyResponseNodeWakeUp();
		proxyResponsePowerOn.setSuccess(success);
		proxyResponsePowerOn.setMessage(message);
		return proxyResponsePowerOn;
	}
