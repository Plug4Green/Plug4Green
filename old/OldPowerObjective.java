...
public class PowerObjective implements btrplace.solver.choco.constraint.CObjective {
	...
	public IntVar getPIdle(ReconfigurationProblem m) {
		...
		IntVar Pidle = VariableFactory.bounded("Pidle", 0, Integer.MAX_VALUE / 100, solver);
    	
        Node[] nodes = m.getNodes();
        //IntDomainVar[] PIdleServer = new IntDomainVar[nodes.size()];

        IntVar[] PIdleServer = new IntVar[nodes.length];
		for (int i = 0; i < nodes.length; i++) {
        	
        	//F4GNode f4gNode = (F4GNode)nodes.get(i);		
            int PIdle = powers
            PIdleServer [i] = VariableFactory.enumerated("IdlePowerServer", new int[]{0, f4gNode.getPIdle()}, solver); 
                   
            m.getSourceModel().getMapping().get
            IntConstraintFactory.arithm(VAR, OP, CSTE)
            if(m.getNodeAction(.getFutureOnlines().contains(nodes.get(i))) {
                m.post(solver..eq(f4gNode.getPIdle(), PIdleServer[i])); 
