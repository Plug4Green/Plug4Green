 ...
 * @author ts
 */
public class F4GHDDConstraint extends F4GConstraint {
	 ...
	 */
	public F4GHDDConstraint(ManagedElementSet<Node> nodes, FIT4GreenType metamodel, int x) {
		this.nodes = nodes;
	...
	public boolean equals(Object o) {
		...
		F4GHDDConstraint that = (F4GHDDConstraint) o;
	...
	public String toString() {
		...
		buffer.append("F4GHDDConstraint(");
		buffer.append(x);
		buffer.append(",");
	...
	public void inject(ReconfigurationProblem core) {
		...
		for (int i = 0; i < nodes.size(); i++) {
			cards2[i] = core.getSetModel(nodes.get(i)).getCard();
			core.post(core.leq(cards2[i], getHDDCap(nodes.get(i).getName())/x));
		}
	}

	private int getHDDCap(String nodeName) {
		int i = 0;
				...
				for (ServerType server : Utils.getAllServers(dt)) {
					if (server.getFrameworkID().equals(nodeName))
						for (MainboardType m : server.getMainboard())
							for (HardDiskType hd : m.getHardDisk())
								i += (int) (hd.getStorageCapacity().getValue());
				}
			}
		return i;
	}
    ...
    public boolean isSatisfied(Configuration configuration) {
        ...
        for (Node n : nodes) {
        	if (configuration.getRunnings(n).size() > (getHDDCap(n.getName())/x)){
            	return false;
            }
        }
        return true;
