...
public class LocalScheduler {
    ...
    public boolean checkInvariant() throws ContradictionException {
        ...
        //No more than 1 d-slice is there is an exclusive d-slice.
        ...
        for (int i = 0; i < sortedMinProfile.length; i++) {
            ...
            //In the worst case, at any moment, there can not be a resource consumption > resource capacity
            ...
            if (!dStarts[i].isInstantiated() && !associatedToCSliceOnCurrentNode(i)) {
                ...
                if (lastT != -1) {
                    if (excl == null || /*|| !excl.isInstantiated() || (/*excl.isInstantiatedTo(1) &&*/ i != exclSlice/*)*/) {
                        ...
                        int t = getLivingLineLB(lastT);
                       // ChocoLogging.getBranchingLogger().finest("-->" + me + ": + " + dStarts[i].pretty() + " inf=" + t);
                        dStarts[i].setInf(t);
                    }
                }
            }
            ...
            for (int i = in.nextSetBit(0); i >= 0; i = in.nextSetBit(i + 1)) {
                if (!dStarts[i].isInstantiated() && !associatedToCSliceOnCurrentNode(i) && dStarts[i].getSup() > lastSup) {
                    ...
                    if (excl == null || /*!excl.isInstantiated() || (excl.isInstantiatedTo(1) &&*/ i != exclSlice/*)*/) {
                        ...
                        int x = getLivingLineUB(s);
                        //ChocoLogging.getBranchingLogger().finest("---->" + me + ": " + i + " " + dStarts[i].pretty() + " sup= " + x);
                        dStarts[i].setSup(x);
                    }
                }
            }
    ...
    private void updateCEndsSup() throws ContradictionException {
        for (int i = out.nextSetBit(0); i >= 0; i = out.nextSetBit(i + 1)) {
            if (!cEnds[i].isInstantiated() && !associatedToDSliceOnCurrentNode(i)) {
                //Looking for the last moment the c-slice can stay.
                ...
                for (int x = 0; x < sortedMinProfile.length; x++) {
                    ...
                    if (t >= cEnds[i].getSup()) {
                       ...
                       /* ChocoLogging.getBranchingLogger().finest(me + ":" + cEnds[i].pretty() + " cannot leave at " + t);
                ...
                if (lastT != -1) {
                    int t =getDeadlineUB(lastT);
                    /*if (me == 1 && t == 2) {
                        ChocoLogging.getBranchingLogger().finest("---->" + me + ": " + i + " " + cEnds[i].pretty() + " cEndsSup =" + t);
                        ChocoLogging.flushLogs();
                    } */
